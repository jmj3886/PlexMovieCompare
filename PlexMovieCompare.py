            ...
            for local_movie in local_listing:
                if server_movie['Title'] == local_movie['Title']:
                    ...
                    if server_movie['Library'] != local_movie['Library']:
                        difference = True
                        location = ("%-"+str(max_location_len)+"s -> %s") % (local_movie['Library'], server_movie['Library'])
            if difference:
...
def generate_movie_listings(remote_address, local_address, plex_token):
    ...
    get_plex_library_content(remote_address, plex_token, libraries)
    ...
    for library in remote_libraries:
        for movie in library['Movies']:
            remote_listing.append({"Title":movie["Title"], "Library":library["Title"]})
    for library in local_libraries:
        for movie in library['Movies']:
            local_listing.append({"Title":movie["Title"], "Library":library["Title"]})
    return remote_listng, local_listing
    
if __name__ == "__main__":
    args = CommandLineParser().parse_info()
    plex_compare(generate_movie_listing(args.remote_address, args.local_address, args.api_token))
